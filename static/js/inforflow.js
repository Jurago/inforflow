...
async function fetchDashboardPage() {
    try { return await apiGet('/dashboard'); } catch (e) { return null; }
}
...
async function fetchASN() {
    try { return await apiGet('/asn'); } catch (e) { return null; }
}

async function fetchASNDetail(asn, hours) {
    try {
        const q = new URLSearchParams({ asn: asn || '' });
        if (hours != null && hours > 0) q.set('hours', String(hours));
        return await apiGet('/asn/detail?' + q);
    } catch (e) { return null; }
}

async function fetchCDNPage() {
    try { return await apiGet('/cdn'); } catch (e) { return null; }
}

async function fetchCDNDetail(name, hours) {
    try {
        const q = new URLSearchParams({ name: name || '' });
        if (hours != null && hours > 0) q.set('hours', String(hours));
        return await apiGet('/cdn/detail?' + q);
    } catch (e) { return null; }
}

async function fetchStreamingPage() {
    try { return await apiGet('/streaming'); } catch (e) { return null; }
}

async function fetchStreamingDetail(name, hours) {
    try {
        const q = new URLSearchParams({ name: name || '' });
        if (hours != null && hours > 0) q.set('hours', String(hours));
        return await apiGet('/streaming/detail?' + q);
    } catch (e) { return null; }
}

async function fetchPeersPage() {
    try { return await apiGet('/peers'); } catch (e) { return null; }
}

async function fetchPeersDetail(asn, hours) {
    try {
        const q = new URLSearchParams({ asn: asn || '' });
        if (hours != null && hours > 0) q.set('hours', String(hours));
        return await apiGet('/peers/detail?' + q);
    } catch (e) { return null; }
}

async function fetchRouterPage() {
    try { return await apiGet('/router'); } catch (e) { return null; }
}

async function fetchRouterDetail(ifindex, hours, name) {
    try {
        const q = new URLSearchParams();
        if (ifindex) q.set('ifindex', String(ifindex));
        if (name) q.set('name', name);
        if (hours != null && hours > 0) q.set('hours', String(hours));
        return await apiGet('/router/detail?' + q);
    } catch (e) { return null; }
}
...
function updateSamplingChip(sampling) {
    const el = document.getElementById('sidebar-sampling-chip');
    if (!el) return;
    const s = sampling || {};
    const factor = s.effective != null ? s.effective : s.factor;
    if (factor == null || !(factor > 0)) {
        el.textContent = 'fator —';
        return;
    }
    const mode = s.mode ? ` (${s.mode})` : '';
    el.textContent = `fator ~${Number(factor).toFixed(0)}×${mode}`;
}
...
async function ensureAuth() {
    ...
    try {
        ...
        // Falha de rede: não derruba a sessão; mostra aviso se possível
        const el = document.getElementById('dash-status');
        if (el && !el.dataset.authWarn) {
            el.dataset.authWarn = '1';
            el.innerHTML = '<div class="alert-item alert-warning">Não foi possível validar a sessão (rede). Tentando novamente…</div>';
        }
        return true;
    }
}
...
window.Inforflow = {
    ...
    fetchStats, fetchDashboardPage, fetchFlows, fetchSNMP, fetchBGP,
    fetchASN, fetchASNDetail, fetchCDNPage, fetchCDNDetail,
    fetchStreamingPage, fetchStreamingDetail, fetchPeersPage, fetchPeersDetail,
    fetchRouterPage, fetchRouterDetail,
    fetchHistory, fetchHistoryCompare, fetchAlerts, fetchSampling,
    updateSamplingChip, ensureAuth,
